...
function LoadSummary() {
  ...
  var sheet =
    SpreadsheetApp.openById(SHEET_ID).getSheetByName(SHEET_INFORMATION);
...
function CreateRhysJuniTracker() {
  ...
  var content = GenerateThreadTrackerPost2(
    character,
    "Juniper",
    "ThreadTrackerSPECIFIC"
  );
...
function CreateCharacterThreadTrackerPosts() {
  for (var i = 0; i < ACTIVE_CHARS.length; i = i + 1) {
    ...
    var content = GenerateThreadTrackerPost(
      character,
      "NewThreadTrackerBLANK1"
    );
...
function GenerateThreadTrackerPost2(character, who, template) {
  ...
  var threads = SpreadsheetApp.openById(SHEET_ID)
    ...
    .filter(
      (row) =>
        (row[2] == character[0] && row[3] == who) || row[10] == "RHYS/JUNIPER"
    );
...
function GenerateThreadTrackerPost(character, template) {
  ...
  var completed_threads = threads
    .filter((row) => row[8] == 1 && row[11] != "ONE SHOT")
    .sort(compareDates);
  var dead_threads = threads.filter((row) => row[8] == -1).sort(compareDates);
  var one_shots = threads
    .filter((row) => row[11] == "ONE SHOT")
    .sort(compareDates);
